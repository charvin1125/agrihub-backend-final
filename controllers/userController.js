...
if (!accountSid || !authToken || !twilioPhoneNumber) {
  console.error("Twilio credentials are missing. OTP sending will fail.");
}
...
const sendOTP = async (mobile, otp) => {
  try {
    const formattedMobile = mobile.startsWith("+") ? mobile : `+91${mobile}`;
    await client.messages.create({
      body: `Your AgriHub OTP is ${otp}. Valid for 5 minutes.`,
      from: twilioPhoneNumber,
      to: formattedMobile,
    });
...
const registerUser = async (req, res) => {
  try {
    ...
    const otp = generateOTP();
    otpStore.set(mobile, { otp, data: { firstName, lastName, mobile }, expires: Date.now() + 5 * 60 * 1000 });
    await sendOTP(mobile, otp);
...
const loginUser = async (req, res) => {
  try {
    ...
    const otp = generateOTP();
    otpStore.set(mobile, { otp, userId: user._id, expires: Date.now() + 5 * 60 * 1000 });
    await sendOTP(mobile, otp);
...
const verifyLoginOTP = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      user: { id: user._id, mobile: user.mobile, isAdmin: user.isAdmin },
    });
  } catch (error) {
    console.error("Error in verifyLoginOTP:", error);
    res.status(500).json({ error: error.message });
...
const getAllCustomers = async (req, res) => {
  try {
    ...
    const customers = await User.find({ isAdmin: false }).select("-__v");
